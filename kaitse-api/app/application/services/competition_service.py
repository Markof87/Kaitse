...
class CompetitionService:
    ...
    async def get_by_id(self, competition_id: int) -> CompetitionResponseDTO:
        ...
        async def list(self) -> list[CompetitionResponseDTO]:
            async with self.uow:
                competitions = await self.uow.competitions.list()
        ...
        async def create(self, dto: CompetitionCreateDTO) -> CompetitionResponseDTO:
            async with self.uow:
                ...
                competition = Competition(code=dto.code, name=dto.name, country=dto.country, level=dto.level, organizer=dto.organizer)
